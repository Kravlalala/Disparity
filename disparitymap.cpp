...
DisparityMap::DisparityMap (DisparityMap &object)
{
  ...
  right_img = new QImage;
  disp_map = new QImage;
  if (object.GetLeft ().format () != 0)
    *left_img = object.GetLeft ();
  if (object.GetRight ().format () != 0)
    *left_img = object.GetRight ();
  if (object.GetDispMap ().format () != 0)
    *disp_map = object.GetRight ();
}
DisparityMap::~DisparityMap ()
{
  delete left_img;
...
void DisparityMap::ExtendImage (QImage &output_img, QImage &input_img,
                                int kernel_size)
{
...
void DisparityMap::FindDisparity (QImage &first_img, QImage &second_img,
                                  int disp_min, int disp_max, int kernel_size)
{
  ...
  QImage temp_map (cols - addition * 2, rows - addition * 2,
                   left_img->format ());
  chronograph.start ();
  for (int x = addition; x < rows - addition; x++) {
    ...
    for (int y = addition; y < cols - addition; y++) {
      undef_flag = false;
      if (qRed (left_line[y]) == 0 && qGreen (left_line[y]) == 0 &&
          qBlue (left_line[y]) == 255) {
        continue;
      } else {
        for (int d = disp_min; d <= disp_max; d++) {
          if ((y + d) >= cols - addition) {
            ...
            current_sum = SSD (*left_img, *right_img, SSD_Mat, kernel_size, x,
                               y, d, d1, disp_min);
            if (d == disp_min) {
              min_sum = current_sum;
              d1 = d;
        ...
        if (undef_flag == false) {
          ...
          if ((qRed (right_line[y + d1]) == 0 &&
               qGreen (right_line[y + d1]) == 0 &&
               qBlue (right_line[y + d1]) == 255)) {
            continue;
          } else {
            for (int d = -(d1 + 3); d <= -disp_min; d++) {
              if ((y + d1 + d) < 0) {
                ...
                current_sum = SSD (*right_img, *left_img, SSD_Mat, kernel_size,
                                   x, y + d1, d, d1, disp_min);
                if (d == -(d1 + 3)) {
                  min_sum = current_sum;
                  d2 = d;
                } else {
...
int DisparityMap::SSD (QImage &left_img, QImage &right_img, double *SSD_Mat,
                       int kernel_size, int x, int y, int d, int d1,
                       int disp_min)
{
  QRgb *left_line;
  QRgb *right_line;
